...
back): # background area ---> should be input by the user manually.
    ...
    Ifmean,Ibmean = mean(cv2.calcHist([If],[0],None,[256],[0,256])),\
                    mean(cv2.calcHist([Ib],[0],None,[256],[0,256])) #Taking the mean of the histogram

    F,B =  ones(shape = I.shape),ones(shape = I.shape) #initalizing the foreground/background probability vector

    Im = I.reshape(-1,1) #Coverting the image array to a vector
    ...
    nodes,nodeids = g.add_nodes(m*n),\
                    pic.add_grid_nodes(J.shape) # Adding non-nodes
    pic.add_grid_edges(nodeids,0),\
    pic.add_grid_tedges(nodeids, J, 255-J)
    ...
    F,B = F.reshape(-1,1),B.reshape(-1,1) # converting  to column vector
